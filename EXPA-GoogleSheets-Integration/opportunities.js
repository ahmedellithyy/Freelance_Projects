function updateOpportunityData(){
  ...
  for(let i =0; i < lr; i++){
    try{
      var response = UrlFetchApp.fetch(`https://gis-api.aiesec.org/v2/opportunities/${oppData[i][7]}?access_token=ACCESS_TOKEN`);  
    ...
    Logger.log(i)
        ...
        if(recievedDate.status == "open"){
          ...
          if((oppData[i][41]=="Closed" && oppData[i][4] == "Audited & Not Passed") || oppData[i][3] != "Passed"){
            ...
            UrlFetchApp.fetch(`https://gis-api.aiesec.org/v2/opportunities/${oppData[i][7]}?access_token=ACCESS_TOKEN`,options)
